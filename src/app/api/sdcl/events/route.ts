...
import { fetchSdclLibraryEventsForMonth } from "@/lib/sdcl-library";
...
export async function GET(req: Request) {
  ...
  try {
    const { events, meta } = await fetchSdclLibraryEventsForMonth(
      year,
      monthIndex,
    );
    return NextResponse.json({
      ...
      meta: {
        year,
        month: monthIndex + 1,
        source: "https://sdcl.bibliocommons.com/v2/events",
        ...meta,
